...
# Function to create radar chart with specified scale levels
def create_radar_chart(metrics, title, scale_levels):
    ...
    # Add scale levels to radial axis
    ax.set_rticks(scale_levels)
...
# Define scale levels for radar chart (adjust as needed)
scale_levels = np.linspace(0, 1, num=6, endpoint=True)
for alert_name, avg_metrics in all_alerts_metrics.items():
    create_radar_chart(avg_metrics, alert_name, scale_levels)
